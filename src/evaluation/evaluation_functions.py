...
class ImageFolderDataset(Dataset):
    ...
    def __init__(self, folder_path, transform=None):
        ...
        self.paths = [
            p for p in Path(folder_path).iterdir() if p.suffix.lower() in EXTENSIONS
        ]
...
class Evaluator:
    ...
    def __init__(self, device="cuda"):
        ...
        self.fid_transform = T.Compose(
            [T.Resize((299, 299)), T.ToTensor(), T.Lambda(lambda x: (x * 255).byte())]
        )
    ...
    def _load_images_pil(self, folder_path: str) -> tuple[list, list[str]]:
        ...
        Returns:
            tuple[list, list[str]]: Tuple of (list of PIL Images, list of file path strings)
        """
        dataset = ImageFolderDataset(folder_path)
        return [dataset[i] for i in range(len(dataset))], [
            str(p) for p in dataset.paths
        ]

    def _load_images_tensor(self, folder_path: str, batch_size: int = 64):
        """
    ...
    def _compute_CLIP(self, image: Image.Image, text: str) -> float:
        ...
        Args:
            ...
            text (str): prompt/caption
        Returns:
            float: the computed CLIP score roughly between -1 and 1
        """
    ...
    def compute_CLIP_batch(self, images_dir: str, texts: list[str]) -> dict:
        ...
        Returns:
            dict: the mean CLIP score, the std, and the CLIP scores for a batch of images
        """
    ...
    def compute_FID(
        self, real_images_dir: str, gen_images_dir: str, batch_size: int = 64
    ) -> float:
        """
        Compute the Frechet Inception Distance between two folders of images.

        Args:
            real_images_dir (str): path to the folder with real images
            gen_images_dir (sr): path to the folder with generated images
            batch_size (int): number of images to process at a time

        Returns:
            float: FID score, the lower the better
        """
        fid = FrechetInceptionDistance(feature=2048).to(self.device)
        fid.update(
            self._load_images_tensor(real_images_dir, batch_size).to(self.device),
            real=True,
        )
        fid.update(
            self._load_images_tensor(gen_images_dir, batch_size).to(self.device),
            real=False,
        )
    ...
    def _keyword_in_caption(self, kw: str, caption: str) -> bool:
        ...
        Returns:
            bool: true if the keyword is in the caption, false if it is not
        """
        return bool(re.search(rf"\b{re.escape(kw)}\b", caption.lower()))

    def compute_attribute_clip_table(
        self, images, captions: list[str], attributes: dict
    ) -> pd.DataFrame:
        """
        ...
        if len(pil_images) != len(captions):
            raise ValueError(
                f"Got {len(pil_images)} images but {len(captions)} captions"
            )
        ...
        for group, keywords in attributes.items():
            ...
            matched = [
                (image, caption)
                for image, caption in zip(pil_images, captions)
            ...
            scores = [
                self._compute_CLIP(img, cap)
                for img, cap in zip(matched_images, matched_captions)
            ]
...
def run_evaluation(
    ...
    clip_results = evaluator.compute_CLIP_batch(
        images_dir=generated_images_path, texts=captions
    )
    ...
    # CLIP bias
    if compute_bias:
        bias_df = evaluator.compute_attribute_clip_table(
            generated_images_path, captions, attributes_dict
        )
